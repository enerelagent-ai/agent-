  ...
  return (
    <div className="min-h-screen bg-[#f3f5f7]">
      ...
      <main className="mx-auto max-w-[1000px] px-4 py-7 sm:px-6 lg:px-8">
        ...
        {metrics.history_svg_path && metrics.history_range ? <section className="mt-6 rounded-2xl border border-slate-200 bg-white p-5"><div className="flex flex-wrap items-end justify-between gap-2"><h2 className="text-xl font-black text-[#20334b]">60 өдрийн м² үнийн хөдөлгөөн</h2><span className="text-xs text-slate-500">{metrics.history_range[0]} — {metrics.history_range[1]}</span></div><svg viewBox="0 0 560 90" preserveAspectRatio="none" className="mt-5 h-36 w-full text-[#ff6b35]" role="img" aria-label="М² үнийн хөдөлгөөний график"><path d={`${metrics.history_svg_path} L554,90 L6,90 Z`} fill="currentColor" opacity="0.1" /><path d={metrics.history_svg_path} fill="none" stroke="currentColor" strokeWidth="2.5" vectorEffect="non-scaling-stroke" /></svg><p className="mt-2 text-xs text-slate-500">Идэвхтэй зарын өдрийн медиан хөдөлгөөн. Бодит хэлцлийн үнэ биш.</p></section> : null}
        ...
        {(metrics.nearby_comparables !== undefined || metrics.water_distance_km !== undefined) && <section className="mt-6 grid gap-3 sm:grid-cols-2"><MetricLightCard label="Ойролцоох харьцуулагч хотхон" value={metrics.nearby_comparables !== undefined ? `${metrics.nearby_comparables} · ${metrics.nearby_cheaper ?? 0} нь хямд` : "—"} /><MetricLightCard label="Усны сүлжээнээс зай" value={metrics.water_distance_km !== undefined ? `${metrics.water_distance_km} км · ${metrics.water_name ?? ""}` : "—"} /></section>}

        {metrics.unit_structure?.length ? <section className="mt-6 overflow-hidden rounded-2xl border border-slate-200 bg-white"><div className="p-5"><h2 className="text-xl font-black text-[#20334b]">Байрны бүтэц</h2><p className="mt-1 text-xs text-slate-500">Бүртгэгдсэн заруудаар ажиглагдсан байр, талбайн хүрээ</p></div><div className="overflow-x-auto"><table className="w-full min-w-[520px] text-sm"><thead className="bg-slate-50 text-left text-xs text-slate-500"><tr><th className="px-5 py-3">Өрөө</th><th className="px-5 py-3 text-right">Ажиглагдсан байр</th><th className="px-5 py-3 text-right">Талбайн хүрээ</th></tr></thead><tbody className="divide-y divide-slate-100">{metrics.unit_structure.map((item) => <tr key={item.rooms}><td className="px-5 py-3 font-bold">{item.rooms} өрөө</td><td className="px-5 py-3 text-right font-semibold">{item.observed_units}</td><td className="px-5 py-3 text-right">{item.area_min_sqm}–{item.area_max_sqm} м²</td></tr>)}</tbody></table></div></section> : null}
