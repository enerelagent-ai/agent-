...
export function parsePublicComplexMetrics(html: string): Metrics {
  ...
  const historyPath = html.match(/aria-label="Өдөр тутмын ₮\/м²-ийн зам"[\s\S]*?<path d="([ML\d.,\s-]+)" fill="none"/);
  const nearby = html.match(/<td>Ойролцоох харьцуулагч хотхон<\/td>\s*<td class="r">(\d+)<\/td>\s*<td class="note">(\d+) нь/);
  const water = html.match(/<td>Усны сүлжээнээс зай<\/td>\s*<td class="r">([\d.]+) км<\/td>\s*<td class="note">([\s\S]*?)<\/td>/);
  ...
  const structureSection = html.match(/<h2>Байрны бүтэц<\/h2>([\s\S]*?)(?:<div class="legal">|$)/)?.[1] ?? "";
  return {
    ...
    ...(historyPath ? { history_svg_path: historyPath[1] } : {}),
    ...(nearby ? { nearby_comparables: Number(nearby[1]), nearby_cheaper: Number(nearby[2]) } : {}),
    ...(water ? { water_distance_km: Number(water[1]), water_name: text(water[2]) } : {}),
    ...
    unit_structure: [...structureSection.matchAll(/<td>(\d+) өрөө<\/td>\s*<td class="r">(\d+)<\/td>\s*<td class="r">([\d.]+)[–-]([\d.]+)<\/td>/g)].map((match) => ({ rooms: Number(match[1]), observed_units: Number(match[2]), area_min_sqm: Number(match[3]), area_max_sqm: Number(match[4]) })),
  };
}
