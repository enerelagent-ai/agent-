...
4. Deploy/redeploy Render backend.
5. Deploy/redeploy Vercel frontend.
6. Trigger **Daily scrape** once; confirm `market snapshot: … price groups recorded`.
7. Keep **Weekly listing inventory reconciliation** scheduled; its first verified complete run establishes real `is_active/delisted_at` changes.
